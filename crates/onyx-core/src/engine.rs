...
/// Keep cpal's global WASAPI device enumerator alive for the life of the process.
///
/// cpal's WASAPI backend caches **one process-global `IMMDeviceEnumerator`** in a
/// `static ENUMERATOR: OnceLock<Enumerator>`, created inside the COM apartment of
/// whichever thread first asks for a device. Its COM guard is a `thread_local!`
/// that calls `CoUninitialize()` when that thread exits — which tears the
/// apartment down and leaves the cached interface pointer dangling, forever,
/// because a `OnceLock` is never re-initialised. cpal marks the wrapper
/// `unsafe impl Send + Sync`, so the next device call from *any* thread walks
/// into freed memory: `STATUS_ACCESS_VIOLATION`. Its own comment — "we only need
/// to have COM initialized in the thread we create the objects in" — is the bug.
/// The code is identical in cpal 0.15.3 through 0.18.2, so there is no version
/// to upgrade to.
///
/// This bites whenever the first thread to touch audio is short-lived, which is
/// the normal case for a Tauri command handler listing devices for the settings
/// panel: the pool thread retires, and the next device change crashes the app.
///
/// The fix is to win that race with a thread that never exits. One parked thread
/// costs a few kilobytes and no CPU, and the apartment then outlives every
/// caller. Every entry point below that can reach cpal calls this first.
...
fn anchor_audio_com() {
    ...
    ANCHOR.call_once(|| {
        ...
        let spawned = std::thread::Builder::new()
            .name("onyx-com-anchor".into())
            .spawn(move || {
                // Creates cpal's global enumerator inside *this* thread's
                // apartment. The result is irrelevant; the side effect is the
                // point, and a machine with no sound card is fine here.
                let _ = cpal::default_host().default_output_device();
                let _ = ready_tx.send(());
                // Never return: `CoUninitialize` must never run for this
                // apartment while the process is alive.
                loop {
                    std::thread::park();
                }
            });
        if spawned.is_ok() {
            // Block until the enumerator exists, so no other thread can win the
            // `OnceLock` race in the meantime. If the anchor thread died first,
            // `recv` fails and we carry on exactly as before this existed.
            let _ = ready_rx.recv();
        }
    });
}

/// No-op off Windows: COM does not exist, and both the CoreAudio and ALSA
/// backends build their enumerators per call.
