...
mod tests {
    ...
    fn a_relative_path_is_resolved_against_the_directory_it_came_from() {
        let files = files_from_args(args(["onyx", "take 1.flac"]), Path::new("/sessions/mix"));
        assert_eq!(files, ["/sessions/mix/take 1.flac"]);
        // Absolute paths are left exactly as given: no canonicalisation, so a
        // symlinked library keeps the name the user knows it by.
        let files = files_from_args(args(["onyx", "/music/take.flac"]), Path::new("/sessions"));
        assert_eq!(files, ["/music/take.flac"]);
    }
